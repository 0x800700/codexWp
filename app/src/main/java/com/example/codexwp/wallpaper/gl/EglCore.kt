...
class EglCore {
    ...
    fun init() {
        ...
        eglConfig = configs[0]
        check(eglConfig != null) { "Unable to find EGLConfig" }

        val contextAttribs3 = intArrayOf(
            EGL14.EGL_CONTEXT_CLIENT_VERSION, 3,
            EGL14.EGL_NONE
        )
        eglContext = EGL14.eglCreateContext(eglDisplay, eglConfig, EGL14.EGL_NO_CONTEXT, contextAttribs3, 0)
        if (eglContext != EGL14.EGL_NO_CONTEXT) {
            isGles3 = true
            return
        }

        val contextAttribs2 = intArrayOf(
            EGL14.EGL_CONTEXT_CLIENT_VERSION, 2,
            EGL14.EGL_NONE
        )
        eglContext = EGL14.eglCreateContext(eglDisplay, eglConfig, EGL14.EGL_NO_CONTEXT, contextAttribs2, 0)
